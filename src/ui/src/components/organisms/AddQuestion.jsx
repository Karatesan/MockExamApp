...
const AddQuestion = ({ setAddQuesionWindow }) => {
    ...
    if (errors.length === 0) {
      ...
      try {
        ...
        const response = await fetch(
          "http://localhost:8080/api/admin/addQuestion",
          {
            method: "POST",
            body: formData,
            headers: authHeader(),
        ...
        setQuestionAdded("The question has been added to the database!");
    ...
    <div className={`${styles.flexCenter}`}>
      <div className="bg-[#FFFFFF22] w-full p-4 lg:h-[78.5vh] rounded-[20px] flex flex-col items-center justify-center relative">
        <button
          ...
          title={"Cancel"}
          onClick={() => setAddQuesionWindow(false)}
        >
          <GiCancel />
        </button>
        ...
        {questionFromFileBox && (
          <AddQuestionFromFile
            ...
            setAnswersFile={setAnswersFile}
            questionFromFileBox={questionFromFileBox}
            setQuestionFromFileBox={setQuestionFromFileBox}
        ...
        <div
          className={`w-full lg:min-h-[700px] rounded-[20px] flex flex-col items-center relative ${
            multiInputBox || questionFromFileBox ? "blur-md" : ""
          }`}
        >
          ...
          >
            ...
            >
              <select
                className="w-[200px] h-[50px] rounded-[20px] block text-white border-[1px] border-primary transition-all bg-transparent pl-3 outline-none focus:border-[#7d35ea] appearance-none mr-4"
                id="dropdown"
            ...
            <div className="flex items-center my-2 ml-5">
              ...
              <div className="flex">
                <Button
                  ...
                  buttonSize={1}
                  type="submit"
                  handleClick={() => setMultiInputBox(true)}
                />
                <Button
                  title={"Questions from File"}
                  variation={0}
                  buttonSize={1}
                  type="submit"
                  handleClick={() => {
                    setCurrentFocus("none");
                    setQuestionFromFileBox(true);
                  }}
                />
              </div>
            </div>
